...
export class CommandClient {
    ...
    sendCommand(farmId: string, turbineId: string, command: any): Promise<void> {
        let url_ = this.baseUrl + "/api/Command/farm/{farmId}/windmill/{turbineId}/command";
    ...
    sendAlert(farmId: string, turbineId: string, alert: any): Promise<void> {
        let url_ = this.baseUrl + "/api/Command/farm/{farmId}/windmill/{turbineId}/alert";
        if (farmId === undefined || farmId === null)
            throw new globalThis.Error("The parameter 'farmId' must be defined.");
        url_ = url_.replace("{farmId}", encodeURIComponent("" + farmId));
        if (turbineId === undefined || turbineId === null)
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processSendAlert(_response);
        });
    }

    protected processSendAlert(response: Response): Promise<void> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export class RealTimeClient {
    ...
    getAlert(connectionId: string | undefined): Promise<RealtimeListenResponseOfListOfAlert> {
    ...
    protected processGetAlert(response: Response): Promise<RealtimeListenResponseOfListOfAlert> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
export class UserClient {
    ...
    login(request: LoginRequestDTO): Promise<FileResponse> {
